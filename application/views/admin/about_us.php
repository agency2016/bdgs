                    ...
                    <table id="statetable" class="table table-bordred table-striped">
                        ...
                        <tbody>
                            <?php if($record) {foreach ($record->result() as $result) { ?>
                                <tr>
                                   <td><?php echo $result->a_desc; ?></td>
                                   <td><button onclick="update_item(<?php echo $result->id; ?> ,'<?php echo $result->a_desc; ?>')" class="btn btn-primary btn-xs" data-title="Edit"><span class="glyphicon glyphicon-edit"></span> Edit</button></td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <textarea id="a_desc_edit" value="" class="form-control" rows="3"></textarea>
                    </div>
                </div>
    ...
    <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="add" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
                    <h4 class="modal-title custom_align custom_heading" id="Heading">Add New</h4>
                </div>
                <div class="modal-body">
                 <div class="form-group">
                ...
                <div class="modal-footer ">
                    <button name="submit" onclick="add_aboutus()" type="button" class="btn btn-warning btn-lg" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span> ADD</button>
...
<script>
    function add_aboutus() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>index.php/About_us/add',
            data: user_data,
            async: false,
    ...
    function delete_item(pid) {
        ...
        $('#delet_confirm').on('click', function (e) {
            ...
            if (sure == 1) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo base_url(); ?>index.php/About_us/delete',
                    data: {cat_id: pid},
                    async: false,
                    success: function (result) {
                        $("#delete").modal('hide');
    ...
    function update_item(item,desc) {
       // console.log(item);
        $('#edit').modal('show');return false;
       
        document.getElementById("a_desc_edit").innerHTML = desc;
        ...
        $('#update_confirm_click').click(function () {
            var user_data = {
                edit_id: 1,
                desc: document.getElementById("a_desc_edit").value
            };
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>index.php/About_us/edit',
                data: user_data,
                async: false,
                success: function (result) {
                    if (result) {
